MODULE Module1
    ...
    PROC main2()
        MoveL Offs(middleOfTable, 0, 0, -20), vfast,fine,tGripper\WObj:=wobjTableN;
        WaitTime(3);
    ...
    PROC putPuckSmoothly(robtarget toTarget, num angle)
        ...
        MoveJ Offs(toTarget, 0, 0, gripHeight + 10),vfast,z50,tGripper\WObj:=wobjTableN;
    ...
    PROC stackPucks()
        ...
        FOR puckNum FROM 1 TO numberOfPucks DO
            MoveJ Offs(puck_positions{puckNum}, 0, 0, safeHeight), vfast, z50, tGripper\WObj:=wobjTableN;
            ready_flag:=TRUE;

...
def create_robtargets(gripper_height, rot, cam_pos):
    ...
    """for key in config.puckdict:
        if isinstance(config.puckdict[key]["position"], list):
            print("puckdict list")
        config.puckdict[key]["position"] = [config.puckdict[key]["position"][0] + cam_pos[0], config.puckdict[key]["position"][1] + cam_pos[1]
        config.puckdict[key]["position"][1] = config.puckdict[key]["position"][1] + cam_pos[1]

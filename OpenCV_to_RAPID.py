...
def pixel_to_mm(px_height, gripper_height):
    ...
    pixel_to_mm = mm_height / px_height
...
def transform_positions(trans, rot):
    ...
    for key, value in config.puckdict:
    ...
    for key, value in config.puckdict:
        config.puckdict[value]["position"] = \
            (config.puckdict[value]["position"][0] * math.cos(math.radians(rotation_z)) +
             config.puckdict[value]["position"][1] * math.sin(math.radians(rotation_z)) + trans[0],
             -config.puckdict[value]["position"][0] * math.sin(math.radians(rotation_z)) +
             config.puckdict[value]["position"][1] * math.cos(math.radians(rotation_z)) + trans[1]
             )
        config.puckdict[value]["angle"] += rotation_z
